 ...
 * 开启状态管理器的话由{@link ChannelStatusManager}负责链路和通道的状态管理，通道的状态维护在{@link VariableChannelCache}的MAP_CHANNEL中，key为ip:port，
 * 后续使用的话可以通过{@link NettyGuideAbstract#support(ProjectAbstract)#getNettyServerContext()#getVariableChannelCache()}进行操作通道，下发指令，关闭通道之类的。
